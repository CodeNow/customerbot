...
class CustomerBot extends SlackBot {
  constructor (config) {
    ...
    this.intercom = new Intercom({
      connection: {
        ...
        appApiKey: process.env.INTERCOM_APP_API_KEY
      },
      feedback: [{
      ...
      allowed_tags: [ 'setup', '1-tier', '2-tier', 'demo-tier', 'inactive' ]
    })
    ...
    this.runnable = new Runnable({
      ...
      apiUrl: 'https://api.runnable.io/',
      userContentDomain: 'runnableapp.com'
    })

    this.stripe = new Stripe()
    this.bigpoppa = new BigPoppa()
  }
    ...
    if (this.isChatMessage(message)) {
      ...
      switch (command) {
        ...
        case 'api':
          ...
          break
        case 'bp':
          console.log('Command: bp')
          this.getBpInfo(message)
          break
        case 'topup':
        ...
        case 'enable':
          ...
          this.bigpoppa.enableCustomer(words[0])
            .then(() => this.sendMessage(message.channel, `${words[0]} successfully enabled.`, true))
            .catch(err => this.errorMessage(message, err))
          break
        case 'disable':
          console.log('Command: disable')
          this.bigpoppa.disableCustomer(words[0])
            .then(() => this.sendMessage(message.channel, `${words[0]} successfully disabled.`, true))
            .catch(err => this.errorMessage(message, err))
          break
        case 'deez':
  ...
  sendMessage (channel, text, log) {
    ...
    if (log) {
      this.postMessage(process.env.SLACK_LOG_CHANNEL, text).fail(err => {
        console.log('Error:', err.toString)
      })
    }
  }
  ...
  errorMessage (message, err) {
    this.sendMessage(message.channel, '```' + err + '```')
  }
  ...
  sendHelpInfo (message) {
    ...
    switch (command) {
      case 'tag':
        this.sendMessage(message.channel, '`tag` Tag an organization.\n' +
          '*Usage:*\n\t_tag <organization-name> <ticket number> <notes>_')
        break
      case 'feedback':
      ...
      case 'enable':
      case 'disable':
        this.sendMessage(message.channel, '`enable/disable` Enable/disable organization.\n' +
          '*Usage:*\n\t_disable <organization-name>_')
        break
      case 'bp':
        this.sendMessage(message.channel, '`bp` Get bigPoppa information about organization.\n' +
  ...
  getBpInfo (message) {
    ...
    if (orgName !== null && orgName !== '') {
      this.sendMessage(message.channel, `Gathering BigPoppa information for *${orgName}*`)
      this.bigpoppa.getBigPoppaOrgFromName(orgName)
        .then((org) => {
          delete org.users
          this.sendMessage(message.channel, '```' + JSON.stringify(org, null, '  ') + '```')
        })
        .catch((err) => {
          console.log(err)
          this.sendMessage(message.channel, '`Error` Could not find *' + orgName + '*')
        })
    } else {
      this.sendMessage(message.channel, '`Error` Invalid organization name')
    }
  }
...
` +
  ...
  extendTrial (message) {
    ...
    if (params.length > 1) {
      ...
      this.bigpoppa.getBigPoppaOrgFromName(company)
        .then(org => {
