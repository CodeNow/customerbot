...
class Thing {
  constructor (config) {
    ...
    this.stripe = new Stripe2(config.stripe)
  ...
  sendKillAsgMessage (githubOrgId, environment) {
    return rabbit.publishEvent(
      'organization.disallowed',
      { githubId: githubOrgId.toString() },
      { env: environment }
    )
  }
  ...
  cleanUpUndesiredASGsOnAWS (environment) {
    return this.listAutoScalingGroups({env: environment})
      .tap((groups) => console.log(groups.length))
      .then((groups) => groups.filter((group) => (group.desired < 1)))
      .then((groups) => {
        return groups.map((group) => group.org)
      })
      .then((orgs) => {
        return Promise.all(
          orgs.map((org) =>
            this.sendKillAsgMessage(org, environment)
            // console.log(org)
          )
        )
        .catch(console.log)
        ...
        .then(org => {
          return this.stripe.fetchCustomer(org.stripeCustomerId)
            ...
            .then(subscription => {
              ...
              console.log(this.newTrialEndDate.unix())
